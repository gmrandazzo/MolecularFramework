...
{
  if(argc != 4){
    printf("\nUsage %s file.mol2 forcefield.txt outfield.mol2\n\n", argv[0]);
  ...
  for(int i = 0; i < 2; i++){
    ...
    FieldCalculator(ff, &molecule, 25, vanderwaals, probe_id[i], 0.f, &field);
    for(int j = 0; j < 4; j++){
      printf("\t%f", SumFieldValue(field, steps[j][0], steps[j][1]));
      SumFieldValue(field, steps[j][0], steps[j][1]);
    }
    char *s = concat(3, ptype[i], "_", argv[3]);
    WriteMol2FieldPoints(field, s);
    free(s);
